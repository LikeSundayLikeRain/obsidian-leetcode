// Phase 20 — Section Protection (forked from sectionLockExtension.test.ts at
// v1.3 per D-protect-04). Reruns the v1.0 Phase 5.5 acceptance cases against
// the new `sectionProtectionExtension`. Cases for the fence-CLOSER lock are
// deleted (the v1.3 widget owns the closer + fence-body range via
// `atomicRanges` from `liveModeViewPlugin.ts`); cases for `## Problem` body,
// `## Code` heading + opener + blank-line pocket, `## Techniques` heading,
// `## Notes` heading, malformed-fence fallthrough, the `'leetcode.*'`
// userEvent bypass, and the boundary fix continue to pass byte-for-byte.
// One new positive case asserts that `input.type` inside the fence body now
// passes through the changeFilter (the explicit narrowing).
//
// FILTER-CALLBACK EXTRACTION CONTRACT (Wave 1 GREEN): tests against
// buildSectionProtectionExtension(plugin) iterate the returned Extension array
...
//   - buildSectionProtectionExtension(plugin): Extension — composed CM6 extension
...
import {
  ...
  buildSectionProtectionExtension,
  computeSnapTarget,
  LOCKED_HEADINGS,
  mergeLockedRanges,
} from '../../src/main/sectionProtectionExtension';
...
/**
 * Walk the Extension array returned by buildSectionProtectionExtension(plugin) and
...
describe('computeLockedRanges — pure function', () => {
  ...
  it('does NOT lock fence closer line — Phase 20 D-protect-01 surgical narrowing', () => {
    // v1.3 widget owns the fence range entirely via `atomicRanges` from
    // liveModeViewPlugin.ts; the closer line is no longer protected by the
    // changeFilter. This is the inverse of the v1.0 Phase 5.5 test
    // ("locks fence closer line end-to-end") which was deleted in this
    // fork. The opener-line lock above persists (heading + blank-line
    // pocket + opener cosmetic lock).
    const body = canonicalNoteBody({ fenceLang: 'python' });
    const state = makeStateForLockTests({ body });
    const ranges = computeLockedRanges(state);
    ...
    const closerLineTo = state.doc.line(closerLine).to;

    // Assert that no emitted [from, to] pair fully contains the closer line.
    // (A pair could touch the boundary — e.g., `to === closerFrom` is OK
    // because that's the upstream boundary, not coverage of the closer
    // itself.)
    let closerLocked = false;
    for (let i = 0; i < ranges.length; i += 2) {
      const from = ranges[i] as number;
      const to = ranges[i + 1] as number;
      if (from <= closerFrom && to >= closerLineTo && to > closerFrom) {
        closerLocked = true;
        break;
      }
    }
    expect(closerLocked).toBe(false);
  });
...
// ────────────────────────────────────────────────────────────────────────
// D-04, D-06 — buildSectionProtectionExtension (changeFilter behavior)
// ────────────────────────────────────────────────────────────────────────

describe('buildSectionProtectionExtension — changeFilter behavior', () => {
  ...
  it('returns suppression-range array when user-input transaction touches a locked range — anchor D-04/Filter-drops', () => {
    ...
    const ext = buildSectionProtectionExtension(plugin as never);
  ...
  it('Phase 20 narrowing — input.type that lands ONLY in the fence body passes through (closer/body no longer protected)', () => {
    // v1.3 D-protect-01: the fence body and closer are owned by the widget
    // via `atomicRanges` from `liveModeViewPlugin.ts`. The parent CM6's
    // changeFilter no longer protects them, so an input.type whose ranges
    // overlap ONLY the fence body must pass through. This is the inverse
    // of v1.2 behavior where the closer + body lock would have caused the
    // filter to return a suppression-range list. We verify by computing
    // the suppression list and asserting it does NOT cover the fence body
    // line (the same shape the filter returns).
    metadataCache.setFrontmatter(filePath, { 'lc-slug': 'two-sum' });
    const plugin = createFakePlugin({ metadataCache });
    const state = makeStateForLockTests({
      body: canonicalNoteBody(),
    ...
    // result is the flat suppression-range array (because other locked
    // headings are present in the doc). Locate the fence body line and
    // assert no pair fully contains it.
    expect(Array.isArray(result)).toBe(true);
    const ranges = result as number[];

    // Locate the fence body line (line BETWEEN the opener and closer in
    // canonicalNoteBody — `class Solution: pass`).
    let openerLine = -1;
    let inCode = false;
    for (let i = 1; i <= state.doc.lines; i++) {
      const t = state.doc.line(i).text;
  ...
  it('Phase 20 alias — computeProtectedRanges is the canonical name for computeLockedRanges', () => {
    // The fork exposes both names via the `export { ... as computeProtectedRanges }`
    // alias in src/main/sectionProtectionExtension.ts. They MUST resolve to
    // the same function (same identity) so the test surface and any future
    // production caller that imports the canonical name receive identical
    // behavior.
    expect(computeProtectedRanges).toBe(computeLockedRanges);
  });
});
