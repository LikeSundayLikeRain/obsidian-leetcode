...
verification: "RESOLVED 2026-05-22 after 3 iterations. Iteration 1 (bubble-phase EditorView.domEventHandlers) failed — listener never fired. Iteration 2 (capture-phase document/window/contentDOM listeners) confirmed via diagnostic logging that our handler DID fire and ran toggleLineComment successfully, but Obsidian's `editor:toggle-comments` ALSO fired in parallel through its own dispatch path. This proved Obsidian's hotkey is dispatched via internal Scope-based keymap manager, NOT a DOM event — DOM-level interception is fundamentally insufficient. Iteration 3 (final, shipped): added `createCmdSlashScopeExtension(app)` in childEditorFactory.ts that uses Obsidian's own Scope mechanism — pushes a Scope on child editor focus, registers Mod-/ inside the Scope to run toggleLineComment, pops Scope on blur. Same pattern Obsidian's Modal class uses to override hotkeys. Plumbed `app` from plugin → buildNestedDecorations → NestedEditorWidget constructor → createChildEditor (made `app` optional only for test fixture compatibility). User confirmed in dev vault: 'wow the test looks good, it works !'. 6 source-level regression tests in childEditorFactory.test.ts assert ViewPlugin shape, Scope push/pop, focus/blur listeners, Mod-/ registration, and cleanup on destroy. Build clean, full vitest 1643/1649 passing."
files_changed:
