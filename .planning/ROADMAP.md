...
**Success Criteria:**
1. With Obsidian's global vim mode ON, pressing j/k/dd/o/i/a/Esc inside the child editor executes in the child editor's vim — never leaks to the parent. DOM-probed `document.activeElement` and the keystroke recipient are consistent. (`:set nu` / `:set nonu` aliases were considered as a bonus enhancement but DROPPED 2026-05-25 per user — runtime toggle conflicts with the read-once-at-mount D-18/D-35 contract; line-number toggling is governed exclusively by the plugin settings + Obsidian's `showLineNumber`. Vim's "unknown option" error on `:set nu` is the accepted behavior.)
