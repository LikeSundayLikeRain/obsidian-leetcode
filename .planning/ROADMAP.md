...
- [ ] **Phase 06: Foundations + Preview Mode** — Lint bump, CI bundle gate, click-to-preview surface; refactor browser row-click into `previewRouter` for downstream reuse.
...
- [ ] 06-01-PLAN.md — Foundations: bump eslint-plugin-obsidianmd to ^0.3.0, fix cascade lint, add Node bundle-size script (500 KB hard / 400 KB soft), bootstrap `.github/workflows/ci.yml`, foundations test stubs.

**Wave 2** *(blocked on Wave 1 completion)*

- [ ] 06-02-PLAN.md — `routeProblemClick` router on LeetCodePlugin + `previewClickBehavior` field on PluginData + new `Preview` settings section with Click-behavior dropdown; `ProblemBrowserView` row-click delegates to the router (shift-click = open).

**Wave 3** *(blocked on Wave 2 completion)*

- [ ] 06-03-PLAN.md — `ProblemPreviewView` ItemView (sticky header + chips + Start/Open buttons) with tab-reuse, right-click context menu, `open-in-preview` palette command, `toDetailCacheEntry` export, scoped CSS chrome.

**Wave 4** *(blocked on Wave 3 completion)*

- [ ] 06-04-PLAN.md — README v1.1 docs: click-default change + click-behavior toggle + right-click + palette docs + Bundle size subsection (500 KB ceiling, 400 KB warn, verified baseline).
...
| 06. Foundations + Preview Mode           | v1.1      | 0/4            | Planned     | -           |
