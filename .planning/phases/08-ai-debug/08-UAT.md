...
[testing complete — Test 1 blocked on Phase 08.1 streaming-transport-fix; Test 2 passed after run-side AI Debug button gap was patched]
...
reason: "electron.net.fetch unavailable in Obsidian 1.10+ renderer (contextIsolation: true blocks main-process net API). Streaming path never fires; AIClient.invokeStream falls through to buffered (requestUrl) on every call. Live MarkdownRenderer flicker check is not testable until streaming transport is fixed. Tracked: .planning/todos/pending/streaming-transport-fix.md (resolves_phase: 08.1)."
...
result: pass
notes: |
  Issue surfaced during dogfood — Run-mode sample-test failures (Wrong Answer with case tabs)
  did NOT show the AI: Debug button initially. Plan 08-05 only wired the button into
  renderSubmitVerdict + (indirectly) the Submit-side ce/re paths via classifyStatus union.
  renderRunResult (case-tabs path) and renderRunErrorBlock (Run-side compile/runtime errors)
  did not receive `onOpenAIDebug` and had no button. Root cause: missing test coverage in
  Plan 08-05 for the two Run-side render branches. Patched in commit (forthcoming) — both
  branches now thread onOpenAIDebug through and render the button when:
    - renderRunResult: any per-case FAIL (aggregatePass === false)
    - renderRunErrorBlock: always (compile/runtime error is always non-Accepted)
  All 6 sub-checks now pass after fix + redeploy.
