...
updated: 2026-05-10T22:22:00-07:00
...
result: pending (deferred to Phase 5.1)
notes: |
...
### G1: Edit-mode inline Run/Submit buttons (UAT-4)
status: pending
severity: ship-blocker
resolves_phase: "5.1"
description: |
  Phase 5.1 must ship Run/Submit buttons anchored inline below the `## Code` fenced
  block in Edit Mode (Source + Live Preview) without layout corruption. See UAT-4
  for full context including failed approaches from 05-05 live smoke.

### G2: Past Submissions pill labels missing (UAT-3)
status: pending
severity: medium-polish
resolves_phase: "5.1 or 5.2"
description: |
  Submission-status pills in the Past Submissions modal render as colored-only shapes
  (no verdict letter/label), except for "Compile Error" which does render text. Users
  cannot distinguish WA / TLE / MLE / RE from each other since all four share red.
  See UAT-3 for investigation pointer.
